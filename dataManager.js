...
var dockerApiCommand = "curl --unix-socket /var/run/docker.sock -H Content-Type: application/json -X GET http://v1.29/networks/"
 exec(dockerApiCommand, function (error, stdout, stderr) {
   if(error === null){
     var data = stdoutToJson(stdout);
     networks = data;
   }
   else {
       console.log('exec error in updateNetwork(): ' + error);
   }
});
}

function updateServicesConf(){
var dockerApiCommand = "curl --unix-socket /var/run/docker.sock -H Content-Type: application/json -X GET http://v1.29/services"
 exec(dockerApiCommand, function (error, stdout, stderr) {
   if(error === null){
     var data  = stdoutToJson(stdout);
     servicesConf = data;
       ...
       console.log('exec error in updateServicesConf: ' + error);
   }
});
}
...
function updateServicesLogs(){
 ...
 servicesConf.forEach( service => {
   var dockerApiCommand = "curl --unix-socket /var/run/docker.sock -H Content-Type: application/json -X GET http://v1.29/services/" + service["ID"]
+"/logs?stderr=true&timestamps=true&tail=50"
    exec(dockerApiCommand, function (error, stdout, stderr) {
      if(error === null){
        logs[service["Spec"]["Name"]] = stdout;
      }
      else {
          console.log('exec error in updateServicesLogs: ' + error);
      }
     });
    });
